...
class TemplateSetListView extends Component {
  ...
  render() {
    ...
    const columns = [
      ...
      { Header: lngStr('training.weeksInCycle'), accessor: data => { return String(data.templates.length); } }
    ];

    return (
      <div className="spaceTop">
        {!hasData && (<p><em>{lngStr('training.noCycles')}</em></p>)}
        {hasData && (
        ...
        <Container fluid>
          ...
          <Row>
            <Col xs={8}>
              <InputText label={lngStr('training.newCycleName')} propName="name" onChange={this.onValueChange} initialValue={this.state.newSet.name} />
            </Col>
            <Col xs={2}>
              <Button color="primary" onClick={() => this.createTemplateSet()}>{lngStr('button.create')}</Button>
            </Col>
          </Row>
        </Container>
      </div>
